...
impl ResourceManager {
    ...
    /// Compare les fichiers locaux et distants
    async fn compare_files(
        ...
        // Scanner les fichiers locaux directement dans local_base
        // (sans créer de sous-dossier server_subdir)
        let local_files = if local_base.exists() {
            Self::scan_local_dir(local_base, local_base)?
        } else {
            HashMap::new()
        };
        ...
        // Vérifier chaque fichier distant
        for (remote_path, remote_info) in &remote_files {
            if let Some(local_info) = local_files.get(remote_path) {
                // Fichier existe localement, vérifier le hash
                if local_info.hash == remote_info.hash && local_info.size == remote_info.size {
                    status.insert(remote_path.clone(), SyncStatus::UpToDate);
                } else {
                    status.insert(remote_path.clone(), SyncStatus::Modified);
                }
            } else {
                // Fichier manque localement
                status.insert(remote_path.clone(), SyncStatus::Missing);
            }
        }
        
        // Identifier les fichiers locaux qui ne sont pas sur le serveur
    ...
    /// Si `force_install` est true, on réinstalle tout même si ça existe
    pub async fn sync_directory(
        ...
        let status = self.compare_files(server_path, local_base).await?;
    ...
    pub async fn sync_game_resources(&self) -> Result<SyncResult, String> {
        ...
        // 1. Synchroniser le dossier base (installation unique si absent)
        // Synchroniser base avec force_install=true pour installer tout si rien n'existe
        ...
        // 2. Synchroniser le dossier updates (toujours)
    ...
    pub async fn check_resources_up_to_date(&self) -> Result<bool, String> {
        ...
        let status = self.compare_files(&self.config.server_updates_path, &game_dir).await?;
